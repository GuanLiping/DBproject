...
public class supplierRegister extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		PrintWriter out=response.getWriter();
		  ...
		  String sqls="select * from Supplier order by id";
		  String sqla="select * from Address";
		  String sqlsNum="SELECT COUNT(*) FROM Supplier";
		  String sqlaNum="SELECT COUNT(*) FROM Address";
		  
		  String sql1="insert into Supplier values(?,?,?,?,?,?)";
		  String sql2="insert into Address values(?,?,?,?,?)";
	    	  ...
	    	  ps=ct.prepareStatement(sqlsNum);
	    	  rs=ps.executeQuery();    	 
	    	  if(rs.next()){    		  
	    		  sNum=rs.getInt(1);
			  ...
			  //request.setAttribute("Aid", address_id);
